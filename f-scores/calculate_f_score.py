...
def write_to_csv(filename, precision, recall, f1_score, f0_5_score, specificity):
    with open(filename, 'w', newline='') as file:
        ...
        writer.writerow(["Precision", "Recall", "F1 Score", "F0.5 Score", "Specificity"])
        writer.writerow([precision, recall, f1_score, f0_5_score, specificity])

def parse_arguments():
    parser = argparse.ArgumentParser(description='Calculate f-scores for a given CSV file.')
...
def main():
    ...
    precision, recall, f1_score, f0_5_score, specificity = calculate_metrics(true_pos, false_pos, false_neg, true_neg)
    print(f"Precision: {precision}\nRecall: {recall}\nF1 Score: {f1_score}\nF0.5 Score: {f0_5_score}\nSpecificity: {specificity}")
    write_to_csv(args.output, precision, recall, f1_score, f0_5_score, specificity)
